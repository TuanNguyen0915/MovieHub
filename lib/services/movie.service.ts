...
export const getAllMovies = async () => {
  const url = "https://api.themoviedb.org/3/trending/movie/week?language=en-US&include_adult=false"
...
export const getMoviesByGenre = async (genreId: any) => {
  const url = `https://api.themoviedb.org/3/discover/movie?include_adult=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${genreId}`
  const { data } = await axios.get(url, OPTIONS)
  return data.results
}
